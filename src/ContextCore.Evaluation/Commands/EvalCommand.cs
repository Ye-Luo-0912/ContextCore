...
public static partial class EvalCommand
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrEmpty(subcommand) || !registry.Contains(subcommand))
        {
            PrintUsage();
            return;
        }

        if (await TryDispatchSubcommandAsync(service, args, subcommand, cancellationToken).ConfigureAwait(false))
        {
            return;
        }
