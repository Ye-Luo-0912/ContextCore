...
internal static class RetrievalPackingPolicy
{
    ...
    /// 实际的 TopK 预留配额在 <see cref="Pack"/> 阶段实施；
    ...
    /// 当 <paramref name="relationOnlyCandidateIds"/> 非空时，为 relation-only 候选
        ...
        IReadOnlySet<string>? relationOnlyCandidateIds = null)
    {
        ...
        // Relation quota 语义修正——Pack 阶段显式为 relation-only 候选预留 TopK 名额。
