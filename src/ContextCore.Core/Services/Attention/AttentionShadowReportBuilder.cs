...
internal static class AttentionShadowReportBuilder
{
    ...
    /// 构建注意力阴影报告。
    /// </summary>
    /// <param name="operationId">操作ID，用于唯一标识一个操作。</param>
    /// <param name="request">上下文检索请求对象，包含本次检索的详细信息。</param>
    /// <param name="rankedCandidates">已排序的上下文检索候选列表。</param>
    /// <param name="currentPackingResult">当前打包结果，反映了此次处理的状态。</param>
    /// <param name="attentionScores">注意力评分列表，每个元素代表了一个候选项的注意力分数。</param>
    /// <returns>返回构建好的注意力阴影报告实例。</returns>
