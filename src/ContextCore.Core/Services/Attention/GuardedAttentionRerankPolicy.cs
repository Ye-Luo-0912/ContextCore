...
internal sealed class GuardedAttentionRerankPolicy
{
    ...
    /// 构建选定的顺序列表。
    /// </summary>
    /// <param name="order">当前排序的候选列表。</param>
    /// <param name="original">原始候选列表。</param>
    /// <param name="attentionById">每个上下文ID对应的注意力分数字典。</param>
    /// <param name="request">检索请求，可能为null。</param>
    /// <param name="extraReasons">额外的原因列表。</param>
    /// <returns>返回一个只读列表，包含重新排序后的AttentionRerankOrderItem项。</returns>
