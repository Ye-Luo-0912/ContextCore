...
/// <summary>规则型 intent detector；不调用 LLM，也不执行 retrieval。Planning 退回空壳后保留为共享意图标签与关键词分类器。</summary>
public sealed class PlanningIntentDetector
{
    ...
    public PlanningIntentDetection Detect(string? currentInput, string? requestedMode = null)
    {
