...
/// </summary>
/// <remarks>
/// 当注入的 <see cref="IContextStore"/> 实现 <see cref="ITransactionalContextStore"/>、
...
/// 摄取阶段持久化 content_hash（SHA-256）与 content_token_cost（精确 token 数）到 Metadata，
...
public sealed class BasicContextIngestionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // 摄取阶段计算 content_hash（SHA-256 小写 hex），与 ContextItem.Checksum 一致。
        ...
        // 若所有参与 store 都实现事务能力接口且工厂可用，走事务路径；否则走原有无事务路径。
    ...
    /// <summary>
    /// 检测注入的 store 是否全部支持事务能力，且事务作用域工厂可用。
    ...
    private async Task IngestWithTransactionAsync(ContextItem normalized, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // 与 ReconcileIngestRelationsAsync 保持一致——无 writer 时跳过整段关系 reconcile，
            ...
            // 只删除由 ingest 投影器生产的边（Provenance="ingest"），保留其他 projector 产生的边。
        ...
        CancellationToken cancellationToken)
    {
        ...
        // 只删除由 ingest 投影器生产的边（Provenance="ingest"），保留人工或其他 projector 产生的边。
