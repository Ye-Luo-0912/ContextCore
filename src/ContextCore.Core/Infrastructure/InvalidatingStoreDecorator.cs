...
public sealed partial class InvalidatingContextStoreDecorator
{
    ...
    /// <summary>
    /// 事务作用域内保存条目。委托给底层 store 的事务重载，<b>不触发缓存失效</b>——
...
/// 透传 <see cref="ITransactionalRelationStore"/> 能力接口，让事务路径在 Decorator 包装下仍可被检测到。
/// </summary>
...
public sealed partial class InvalidatingRelationStoreDecorator
{
    ...
    /// <summary>
    /// 事务作用域内批量 upsert。委托给底层 store 的事务重载，<b>不触发缓存失效</b>——
    ...
    /// <summary>
    /// 事务作用域内删除单条关系。委托给底层 store 的事务重载，<b>不触发缓存失效</b>——
    /// 事务尚未 Commit。失效应在外层 scope.CommitAsync 成功后由调用方触发。
    /// </summary>
    public async Task<bool> DeleteAsync(
        string workspaceId,
