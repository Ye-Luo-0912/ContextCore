...
public sealed class CompositeContextEventSink : IContextEventSink
{
    ...
    public CompositeContextEventSink(IEnumerable<IContextEventSink> sinks)
    {
        ...
        // 复合接收器的 Kind 取所有子 sink 的最严格值——只要有一个子 sink 为 Required，
    ...
    /// <summary>
    /// 取所有子 sink 的最严格 Kind。只要有一个子 sink 为 Required，复合接收器即为 Required。
    ...
    /// <summary>
    /// 批量写入转发。每个子 sink 独立 try/catch，与 <see cref="EmitAsync"/> 保持
