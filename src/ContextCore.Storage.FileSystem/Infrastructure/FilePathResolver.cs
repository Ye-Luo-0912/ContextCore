...
public sealed class FilePathResolver
{
	...
	public string GetRawContentPath(
		...
		string collectionId,
		string itemId,
		Abstractions.ContextContentFormat format)
	{
		var ext = format switch
		{
			Abstractions.ContextContentFormat.Markdown => ".md",
			Abstractions.ContextContentFormat.Json => ".json",
			Abstractions.ContextContentFormat.Yaml => ".yaml",
			Abstractions.ContextContentFormat.Xml => ".xml",
			Abstractions.ContextContentFormat.Html => ".html",
			_ => ".txt"
		};
	...
	public string GetShortTermRawEventsJsonlPath(string workspaceId, string collectionId)
		=> Path.Combine(GetShortTermDirectory(workspaceId, collectionId), "raw-events.jsonl");

	/// <summary>获取短期工作项 JSONL 文件路径。</summary>
	public string GetShortTermWorkingItemsJsonlPath(string workspaceId, string collectionId)
		=> Path.Combine(GetShortTermDirectory(workspaceId, collectionId), "working-items.jsonl");
	...
	public string GetShortTermArchivedRawEventsJsonlPath(string workspaceId, string collectionId)
		=> Path.Combine(GetShortTermArchiveDirectory(workspaceId, collectionId), "raw-events.jsonl");
	...
	public string GetShortTermArchivedWorkingItemsJsonlPath(string workspaceId, string collectionId)
		=> Path.Combine(GetShortTermArchiveDirectory(workspaceId, collectionId), "working-items.jsonl");
	...
	public string GetShortTermCompactionRunsJsonlPath(string workspaceId, string collectionId)
		=> Path.Combine(GetShortTermDirectory(workspaceId, collectionId), "compact-runs.jsonl");
