...
/// </summary>
/// <remarks>
/// 旧实现使用静态 ConcurrentDictionary&lt;string, SemaphoreSlim&gt;，
...
/// 修复 retired-entry 竞态——原实现 GetOrAdd 返回 entry 与 lock(entry){RefCount++}
...
public sealed class FileLockProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // GetOrAdd + RefCount++ 必须原子化，否则并发释放者可在两者之间回收 entry，
    ...
    /// <summary>
    /// 释放租约时调用。释放内存信号量并递减引用计数；
    /// 若 RefCount 归零（无等待者），从静态字典 CAS 移除 LockEntry，避免锁对象无限增长。
    /// RefCount 递减 + 回收判定统一在 lock(LocalLocks) 下完成，与 Acquire 路径使用同一锁，
    ...
    /// <summary>
    /// 统一 RefCount 递减 + 回收逻辑。在 lock(LocalLocks) 下完成，
...
/// <summary>
/// 引用计数的锁条目。RefCount 在 lock(this) 保护下访问。
...
public sealed class FileWriteLease : IAsyncDisposable
{
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        // 通过 ReleaseEntry 统一释放信号量 + 递减引用计数 + 可能回收。
