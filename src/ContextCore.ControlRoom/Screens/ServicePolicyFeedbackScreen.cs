...
/// <summary>Service 模式下的 Policy Feedback Dataset 只读页面。</summary>
public static class ServicePolicyFeedbackScreen
{
        ...
        CancellationToken cancellationToken = default)
    {
        while (true)
        {
            try
            {
                var snapshot = await service.GetServicePolicyFeedbackDatasetSnapshotAsync(cancellationToken: cancellationToken)
                    .ConfigureAwait(false);
                Console.WriteLine(ServiceOperationalRenderer.RenderPolicyFeedbackDataset(snapshot));
            }
