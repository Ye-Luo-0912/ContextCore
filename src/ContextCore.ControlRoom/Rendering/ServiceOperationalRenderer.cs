...
public static class ServiceOperationalRenderer
{
    ...
    public static string RenderLearningFeatures(ServiceLearningFeaturesSnapshot snapshot)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("V7.13 Scoped Runtime Preview Live Activation Execution Gate");
        var v7LiveExec = snapshot.ScopedRuntimePreviewLiveActivationExecutionReport;
        if (v7LiveExec is null)
        {
            builder.AppendLine("- status : not generated");
            builder.AppendLine("- path   : vector/v7/live-activation-execution.json");
            builder.AppendLine("- action : run eval scoped-runtime-preview-live-activation-execution");
        }
        else
        {
            builder.AppendLine($"- execution gate     : {v7LiveExec.ExecutionGatePassed}");
            builder.AppendLine($"- gate passed        : {v7LiveExec.GatePassed}");
            builder.AppendLine($"- recommendation     : {v7LiveExec.Recommendation}");
            builder.AppendLine($"- next phase         : {v7LiveExec.NextAllowedPhase}");
            builder.AppendLine($"- execute live       : {v7LiveExec.ExecuteLiveActivation}");
            builder.AppendLine($"- plan id matches    : {v7LiveExec.PlanIdMatches}");
            builder.AppendLine($"- killSwitch armed   : {v7LiveExec.KillSwitchArmed}");
            builder.AppendLine($"- configPatchWritten : {v7LiveExec.ConfigPatchWritten}");
            builder.AppendLine($"- runtimeActivation  : {v7LiveExec.RuntimeActivation}");
            builder.AppendLine($"- invariants         : formalRetrieval={v7LiveExec.FormalRetrievalAllowed} formalPkg={v7LiveExec.FormalPackageWritten} noRuntimeMutation={v7LiveExec.NoRuntimeMutationInvariant}");
            if (v7LiveExec.BlockedReasons.Count > 0)
                builder.AppendLine($"- blocked            : {string.Join(", ", v7LiveExec.BlockedReasons)}");
        }

        builder.AppendLine();
        builder.AppendLine("V7.14 Scoped Runtime Preview Live Activation Observation");
        var v7LiveObs = snapshot.ScopedRuntimePreviewLiveActivationObservationReport;
        if (v7LiveObs is null)
        {
            builder.AppendLine("- status : not generated");
            builder.AppendLine("- path   : vector/v7/live-activation-observation.json");
            builder.AppendLine("- action : run eval scoped-runtime-preview-live-activation-observation");
        }
        else
        {
            builder.AppendLine($"- observation passed : {v7LiveObs.ObservationPassed}");
            builder.AppendLine($"- gate passed        : {v7LiveObs.GatePassed}");
            builder.AppendLine($"- recommendation     : {v7LiveObs.Recommendation}");
            builder.AppendLine($"- next phase         : {v7LiveObs.NextAllowedPhase}");
            builder.AppendLine($"- requests           : {v7LiveObs.ObservedRequestCount}/{v7LiveObs.MaxRequestCap}");
            builder.AppendLine($"- approved requests  : {v7LiveObs.ApprovedScopeRequestCount}");
            builder.AppendLine($"- non-approved noOps : {v7LiveObs.NonApprovedScopeNoOpCount}");
            builder.AppendLine($"- killSwitch trips   : {v7LiveObs.KillSwitchTripCount}");
            builder.AppendLine($"- appliedDeltaZero   : {v7LiveObs.AppliedDeltaZero}");
            builder.AppendLine($"- configPatchWritten : {v7LiveObs.ConfigPatchWritten}");
            builder.AppendLine($"- runtimeActivation  : {v7LiveObs.RuntimeActivation}");
            builder.AppendLine($"- invariants         : noRuntimeMutation={v7LiveObs.NoRuntimeMutationInvariant}");
            if (v7LiveObs.BlockedReasons.Count > 0)
        ...
        builder.AppendLine();
        builder.AppendLine("V8.1 Formal Retrieval Promotion Plan");
        var v8Plan = snapshot.FormalRetrievalPromotionPlanReport;
        if (v8Plan is null)
        {
            builder.AppendLine("- status : not generated");
            builder.AppendLine("- path   : vector/v8/formal-retrieval-promotion-plan.json");
            builder.AppendLine("- action : run eval formal-retrieval-promotion-plan");
        }
        else
        {
            builder.AppendLine($"- plan passed        : {v8Plan.PlanPassed}");
            builder.AppendLine($"- gate passed        : {v8Plan.GatePassed}");
            builder.AppendLine($"- recommendation     : {v8Plan.Recommendation}");
            builder.AppendLine($"- next phase         : {v8Plan.NextAllowedPhase}");
            builder.AppendLine($"- retrieval blocked  : {v8Plan.FormalRetrievalStillBlocked}");
            builder.AppendLine($"- manual approval    : {v8Plan.RequiredManualApproval}");
            builder.AppendLine($"- abort conditions   : {v8Plan.AbortConditions.Count}");
