...
//
// 设计原则（对齐用户澄清）：
// 1. Utility Ledger：新增独立 Store 但由 Trace/Event 异步批量物化。
// - IUtilityLedgerStore 是 read-only API：Query / GetLatestEntry / GetExpertContributions。
// - 写入路径：Trace/Event materializer 异步批量写入。
// - 不暴露 WriteAsync / AppendAsync 等方法到接口（避免误用为同步写）。
// 2. Router 标签：Expert-level ablation 不做全量 candidate LOO。
// - Utility Ledger 提供数据基础，但 LOO 计算在更上层执行（不在此 store）。
// 3. Expert 重叠：该 Expert 特征贡献只有无其他来源才删 Candidate。
// - ConflictSet 记录哪些 Candidate 由多个 Expert 重叠贡献，便于 ablation 决策。
// 4. 交互归因：普通 LOO + 困难 pair ablation + 少量近似 Shapley。
// - Utility Ledger 提供 per-Expert 贡献数据；近似 Shapley 在更上层执行。
// 5. Learning Loop 硬边界：不直接用 selected=positive、dropped=negative。
// - Utility Ledger 记录所有 candidate 的 utility（无论 selected/dropped），
// 避免将 dropped 简单视为负样本。
//
// 与 已有契约的关系：
// - RetrievalExpert：Utility Ledger 按 Expert 分组
// - ExpertRoutingDecisionSet：路由决策影响 per-Expert TopK
// - ContextCandidateEnvelope.Utility.CandidateUtilityScore：
// Utility Ledger 物化 CandidateUtilityScore 的历史快照
// - MemoryStateEventRecord：ConflictSet 可关联 memory state 转换事件
...
/// 5. 硬边界：所有 candidate（selected/dropped）都写入 ledger，
...
/// 4. 硬边界：所有 candidate（selected/dropped）都应写入 ledger，避免"dropped 视为负样本"的简化。
/// </remarks>
...
public enum ConflictSetKind : byte
{
    ...
    /// <summary>同 ItemId 来自不同 Expert（Expert 重叠；澄清 的核心场景）。</summary>
...
/// 4. 用于 ablation / 归因分析（澄清）：识别哪些 candidate 互相竞争，
/// 哪些 Expert 重叠贡献。
/// </remarks>
public sealed record ConflictSet
{
    ...
    /// <summary>解决状态（默认 Unresolved；materializer 选定 resolved item 时为 AutoResolved）。</summary>
    public ConflictResolutionStatus ResolutionStatus { get; init; } = ConflictResolutionStatus.Unresolved;

    /// <summary>"获胜权威"来源（对齐用户规格的"chosen authority"字段）。
    ...
    /// <summary>解决时间戳（仅当 ResolutionStatus != Unresolved 时填充）。</summary>
    public DateTimeOffset? ResolvedAt { get; init; }

    /// <summary>解决者（仅当 ResolutionStatus=ManuallyResolved 时填充）。</summary>
...
public sealed record ConflictSetQuery
{
    ...
    /// <summary>按 ResolutionStatus 过滤（可空 = 不限制）。</summary>
...
// 4. 硬边界延续：用户反馈是观察信号而非训练标签；导出器需保留"无反馈"样本以避免偏差。
