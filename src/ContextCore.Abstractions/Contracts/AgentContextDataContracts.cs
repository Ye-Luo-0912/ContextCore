...
//
// 目标（对齐用户规格第六节）：
// 定义 AgentContextSnapshotRef.ContentJson 的实际结构，
...
//
// 与 Bounded Context Orchestrator 的关系：
// - AgentContextSnapshot 可包含 ContextDecisionResult + PackageQualityReport 引用 ID。
// - AgentContextDelta 由修复操作产生（如修复后 snapshot 与原 snapshot 的差异）。
// - 但 Agent 数据契约不直接引用 Orchestrator 类型（避免循环依赖）。
// ===========================================================================
...
/// 4. Delta 可由修复操作产生（修复后 snapshot 与原 snapshot 的差异）。
/// </remarks>
