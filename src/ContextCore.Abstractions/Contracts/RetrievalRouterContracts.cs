...
//
// 设计原则（用户澄清 #5 / #6）：
//   1. Router 不替换 HybridContextRetriever；R20-2 阶段 Router 作为可选
...
//   4. Budget-Aware TopK 模拟（用户澄清 #6）：V1 简化为"非 Mandatory
//      Expert 平均分配总预算"；V2+ 可加入 per-Expert 质量—成本曲线模型。
//   5. Expert-level ablation（用户澄清 #5）：Router 通过 Mask 接受
...
//
// 与 R19-3 Engine 集成的关系：
//   Engine.DecideAsync 在 R19-3 阶段直接读取 bundle.Routing/Bundle.Budget
//   应用到 envelope 决策。R20-2 引入 Router 后，Engine 可以在 DecideAsync
//   之前调用 Router.Route(...) 得到 per-Expert 决策集合，再据此对 envelope
//   分组应用 TopK / TokenBudget 上限。R20-2 阶段不强制 Engine 调用 Router；
